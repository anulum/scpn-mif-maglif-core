...
# SCPN MIF MagLIF Core — diagnostic and clock semantics tests
...
from scpn_mif_maglif_core.errors import DiagnosticPlanError
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic liner-arrival marker",
    "provenance": "synthetic fixture",
...
NONCYCLIC_BINDINGS = {
    ...
    "clock_epoch": "clk_shot",
    "coordinate_frame": "frm_liner_axis",
    "provenance": "synthetic fixture",
    "quality": "synthetic quality flags",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        identifier="frm_liner_axis",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="liner-axis cylindrical frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_COMPRESSION_TRAJECTORY_SET = (
    SignalDeclaration(
        identifier="sig_liner_radius",
        quantity="length",
        unit="m",
        role=SignalRole.CARRIER,
        description="synthetic liner radius trajectory",
    ),
    SignalDeclaration(
        identifier="sig_liner_velocity",
        quantity="velocity",
        unit="m/s",
        role=SignalRole.AUXILIARY,
        description="synthetic liner velocity",
    ),
)
SIGNALS_CH_LINER_ARRIVAL_TRAIN = (
    SignalDeclaration(
        identifier="sig_arrival_speed",
        quantity="velocity",
        unit="m/s",
        role=SignalRole.AMPLITUDE,
        description="synthetic liner arrival speed",
    ),
    SignalDeclaration(
        identifier="sig_liner_cycle",
...
SIGNALS_CH_LINER_ASYMMETRY_SET = (
    SignalDeclaration(
        identifier="sig_asymmetry_amplitude",
        quantity="normalised_amplitude",
        unit="1",
        role=SignalRole.AMPLITUDE,
        description="synthetic liner asymmetry amplitude",
    ),
    SignalDeclaration(
...
def clock_shot() -> ClockModel:
    ...
    return ClockModel(
        ...
        epoch="pulsed-power store trigger t0",
...
def channel_event_train() -> DiagnosticChannelPlan:
    """Build the synthetic liner-arrival event channel."""
    return DiagnosticChannelPlan(
        identifier="ch_liner_arrival_train",
        ...
        acquisition_duration_s=2e-07,
        element_count=1,
        evidence_bindings=dict(EVENT_BINDINGS),
        signals=SIGNALS_CH_LINER_ARRIVAL_TRAIN,
        synthetic=True,
    )


def channel_noncyclic_one() -> DiagnosticChannelPlan:
    """Build the synthetic compression-trajectory channel."""
    return DiagnosticChannelPlan(
        identifier="ch_compression_trajectory_set",
        ...
        signals=SIGNALS_CH_COMPRESSION_TRAJECTORY_SET,
        synthetic=True,
    )


def channel_derived() -> DiagnosticChannelPlan:
    """Build the synthetic liner-asymmetry mode channel."""
    return DiagnosticChannelPlan(
        identifier="ch_liner_asymmetry_set",
        candidate_id="magneto_inertial.resolved_asymmetry_mode",
        carrier=SemanticCarrier.COMPLEX_MODE,
        clock_identifier="clk_facility",
        sample_rate_hz=2.0e09,
        max_signal_frequency_hz=5.0e08,
        timing_uncertainty_s=None,
        acquisition_start_s=0.0,
        acquisition_duration_s=2e-07,
        element_count=8,
        evidence_bindings=dict(DERIVED_BINDINGS),
        signals=SIGNALS_CH_LINER_ASYMMETRY_SET,
        synthetic=True,
    )
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="maglif_reference_plan",
...
def test_candidate_properties_follow_class_tables() -> None:
    ...
    mode = by_id["magneto_inertial.resolved_asymmetry_mode"]
    assert SemanticCarrier.COMPLEX_MODE in mode.admissible_carriers
    assert "observation_operator" in mode.required_evidence
    arrival = by_id["magneto_inertial.driver_arrival"]
    assert SemanticCarrier.EVENT_CYCLE in arrival.admissible_carriers
    assert "timing_uncertainty" in arrival.required_evidence
...
def _transformation(**overrides: Any) -> FrameTransformation:
    ...
    values: dict[str, Any] = {
        "source_identifier": "frm_liner_axis",
...
def test_channel_rejects_unsorted_or_duplicate_signals() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=tuple(reversed(SIGNALS_CH_LINER_ASYMMETRY_SET)))
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(
            signals=(
                *SIGNALS_CH_LINER_ASYMMETRY_SET,
                SIGNALS_CH_LINER_ASYMMETRY_SET[-1],
            )
        )
...
def test_non_event_channel_rejects_timing_marker() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="only event-relative"):
        _derived(signals=(*SIGNALS_CH_LINER_ASYMMETRY_SET, marker))
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_liner_axis")
...
def test_report_flags_cyclic_array_without_amplitude_signal() -> None:
    ...
    channel = _derived(
        signals=tuple(
            signal
            for signal in SIGNALS_CH_LINER_ASYMMETRY_SET
...
def test_channel_rejects_nyquist_violation() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="Nyquist"):
        _derived(sample_rate_hz=2.5e08)
...
def test_plan_accepts_explicit_deferral() -> None:
    ...
    plan = DiagnosticPlan(
        identifier="maglif_partial_plan",
        binding=CATALOGUE_BINDING,
...
def test_report_flags_mhd_band_outside_typical_range() -> None:
    ...
    channel = _derived(sample_rate_hz=1.0e11, max_signal_frequency_hz=5.0e10)
    plan = DiagnosticPlan(
        identifier="maglif_reference_plan",
        binding=CATALOGUE_BINDING,
        clocks=(clock_facility(), clock_shot(), clock_simulation()),
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Slutz" in findings[0].message
...
def test_plan_rejects_unknown_frame_reference() -> None:
    ...
    plan = synthetic_plan()
    kept = tuple(frame for frame in plan.frames if frame.identifier != "frm_liner_axis")

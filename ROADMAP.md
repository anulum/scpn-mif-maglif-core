...
1. **Device configuration model** — typed configuration policy for MagLIF
   (liner/target design classes, three-stage timing envelopes,
   premagnetisation and preheat contracts), with evidence-maturity target
   `computational_prototype`.
2. **Diagnostic and clock semantics** — declared stage, liner, and burn

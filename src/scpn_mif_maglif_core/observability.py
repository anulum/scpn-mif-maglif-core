...
# SCPN MIF MagLIF Core — diagnostic and clock semantics
...
identities for the magnetised-liner inertial-fusion configuration this
...
from scpn_mif_maglif_core.configuration import ConsistencyFinding
...
ASYMMETRY_BAND_HZ: Final = (1.0e06, 1.0e09)
ARRIVAL_TIMING_CEILING_S: Final = 1.0e-09
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-MIF-MAGLIF-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    ...
    CandidateProfile(
        ...
        phenomenon="resolved liner, jet, or target asymmetry mode",
        configurations=("maglif",),
        observability_class=ObservabilityClass.DERIVED_CYCLIC,
    ),
    CandidateProfile(
        candidate_id="magneto_inertial.translation_and_compression",
        phenomenon=("translation, merge, radius, velocity, and compression trajectory"),
        configurations=("maglif",),
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-MIF-MAGLIF-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented MagLIF diagnostic practice; empty
        ...
        for channel in self.channels:
            observability_class = channel.observability_class
            if (
                channel.candidate_id == "magneto_inertial.resolved_asymmetry_mode"
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "Hz is outside the ~100 ns liner "
                            "implosion asymmetry range 1-1000 MHz "
                            "(Slutz 2010)"
                        ),
                    )
                )
            if (
                observability_class is ObservabilityClass.EVENT_RELATIVE
                and channel.timing_uncertainty_s is not None
                and channel.timing_uncertainty_s > ARRIVAL_TIMING_CEILING_S
            ):
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "liner-arrival timing on ~100 ns "
                            "implosions (Slutz 2010)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the MagLIF implosion scale of up to ~1 "
                            "us (Slutz 2010)"
                        ),
                    )
                )
